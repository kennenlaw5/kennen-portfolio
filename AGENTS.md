...
- **Production deployment is deliberately manual.** Render auto-deploy must remain off. `.github/workflows/deploy.yml` reruns `.github/workflows/ci.yml`, waits for approval from the protected GitHub `production` environment, and triggers a commit-specific Render deploy through the environment-scoped `RENDER_DEPLOY_HOOK_URL` secret. Do not add automatic push-based deployment behavior.
